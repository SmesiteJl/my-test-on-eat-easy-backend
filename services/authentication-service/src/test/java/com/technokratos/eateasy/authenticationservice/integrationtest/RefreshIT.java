...
public class RefreshIT {
  ...
  private static final UserDetails USER =
      User.builder()
          ...
          .roles(ROLE)
          .build();
  private static final RefreshTokenEntity TOKEN_ENTITY =
  ...
  void setUp() {
    ...
    token =
        refreshTokenGeneratorService.generate(USER, Map.of(refreshTokenIdClaim, REFRESH_TOKEN_ID));
  ...
  void refreshWithGoodCredentialsShouldSaveNewRefreshToken() {
    ...
    RefreshTokenEntity tokenEntity =
        redisTemplate.opsForValue().get(refreshTokenKeyPrefix + refreshTokenId);
  ...
  void refreshWithGoodCredentialsShouldDeletePreviousToken() {
    ...
    boolean hasKey =
        Objects.requireNonNullElse(
            redisTemplate.hasKey(refreshTokenKeyPrefix + REFRESH_TOKEN_ID), false);
    assertFalse(hasKey);
  }
