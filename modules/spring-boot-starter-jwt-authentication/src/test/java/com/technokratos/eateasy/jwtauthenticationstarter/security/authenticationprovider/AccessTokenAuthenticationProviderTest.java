...
class AccessTokenAuthenticationProviderTest {
  ...
  @Mock private AccessTokenParserService tokenParser;
  @Mock private ConfigurableClaimExtractor<Collection<GrantedAuthority>> authoritiesExtractor;
  @Mock private ConfigurableClaimExtractor<Serializable> userIdExtractor;
  ...
  void setUp() {
    provider =
        AccessTokenAuthenticationProvider.builder()
  ...
  void authenticateValidTokenWithUserIdShouldReturnIdentifiablePrincipal() throws Exception {
    ...
    assertAll(
        ...
        () ->
            assertEquals(
                USERNAME, ((IdentifiableUserDetails<?>) result.getPrincipal()).getUsername()),
        () ->
            assertIterableEquals(
                AUTHORITIES,
                ((IdentifiableUserDetails<?>) result.getPrincipal()).getAuthorities()));
  }

  @ParameterizedTest
  @NullAndEmptySource
  void authenticateValidTokenWithoutUsernameShouldThrowBadCredentialsException(String username)
      throws Exception {
    AccessAuthenticationToken token = AccessAuthenticationToken.unauthenticated(VALID_TOKEN);
  ...
  @Test
  void
      authenticateValidTokenWhenThrowsIllegalArgumentExceptionShouldThrowInternalAuthenticationServiceException()
          throws Exception {
    AccessAuthenticationToken token = AccessAuthenticationToken.unauthenticated(VALID_TOKEN);
  ...
  void authenticateValidTokenWithoutUserIdShouldReturnsBasicPrincipal() {
    ...
    assertAll(
        ...
        () ->
            assertIterableEquals(
                AUTHORITIES, ((UserDetails) result.getPrincipal()).getAuthorities()));
  }
