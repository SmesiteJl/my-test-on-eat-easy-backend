...
class JwtAccessTokenGeneratorServiceTest {
  ...
  private static final Map<String, Object> EXTRA_CLAIMS =
      Map.of(EXTRA_CLAIM_KEY, EXTRA_CLAIM_VALUE);
  ...
  @BeforeEach
  void setUp() {
    identifiableUserDetails =
        new TestIdentifiableUserDetails(USER_ID, USERNAME, PASSWORD, AUTHORITY);
    userDetails = User.withUsername(USERNAME).password(PASSWORD).authorities(AUTHORITY).build();
    service =
        JwtAccessTokenGeneratorService.builder()
  ...
  private static class TestIdentifiableUserDetails implements IdentifiableUserDetails<UUID> {
    ...
    private TestIdentifiableUserDetails(
        UUID id, String username, String password, String authority) {
      this.id = id;
      this.delegate = User.withUsername(username).password(password).authorities(authority).build();
    }
