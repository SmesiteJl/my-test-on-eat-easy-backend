...
class JwtRefreshTokenParserServiceTest {
  ...
  void setup() {
    tokenParser =
        JwtRefreshTokenParserService.builder()
            .jwtParser(jwtParser)
    ...
    when(refreshTokenIdExtractor.extract(tokenWithExceptionClaims))
  ...
  void validateMissingTokenShouldThrowCredentialsExpiredException() {
    assertThrows(
        CredentialsExpiredException.class, () -> tokenParser.validate(MISSING_TOKEN, FINGERPRINT));
  ...
  void validateProvidedNullFingerprintShouldThrowBadCredentialsException() {
    assertThrows(BadCredentialsException.class, () -> tokenParser.validate(VALID_TOKEN, null));
  }
  ...
  void validateExpectedNullFingerprintShouldThrowBadCredentialsException() {
    when(refreshTokenEntity.getFingerprint()).thenReturn(null);
    assertThrows(
        BadCredentialsException.class, () -> tokenParser.validate(VALID_TOKEN, FINGERPRINT));
  }
  ...
  void validateBaseTokenValidationFailureShouldPropagateException() {
    assertThrows(
        BadCredentialsException.class, () -> tokenParser.validate(INVALID_TOKEN, FINGERPRINT));
