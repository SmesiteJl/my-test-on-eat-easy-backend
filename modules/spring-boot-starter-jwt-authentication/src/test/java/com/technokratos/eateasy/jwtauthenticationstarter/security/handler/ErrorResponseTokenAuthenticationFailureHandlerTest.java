...
class ErrorResponseTokenAuthenticationFailureHandlerTest {
  ...
  void determineHttpStatus_returnsCorrectStatus(
      AuthenticationException exception, HttpStatus expectedStatus) {
    assertEquals(expectedStatus, handler.determineHttpStatus(exception));
  }

  @ParameterizedTest
  @MethodSource("exceptionProvider")
  void determineClientMessage_returnsFormattedMessage(
      AuthenticationException exception, HttpStatus ignored, String expectedMessage) {
