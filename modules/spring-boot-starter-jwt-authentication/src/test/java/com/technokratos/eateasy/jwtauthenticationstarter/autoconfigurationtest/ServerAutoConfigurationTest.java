...
public class ServerAutoConfigurationTest {
  ...
  private static final String[] SERVER_MODE_PROPERTIES =
      new String[] {
        "jwt.mode=server",
  ...
  private static final String[] CLIENT_MODE_PROPERTIES =
      new String[] {
        "jwt.mode=client",
        "jwt.issuer=http://localhost:8080",
        "jwt.tokens.access.header=Authorization",
  ...
  private static final String[] OFF_MODE_PROPERTIES =
      new String[] {
        "jwt.mode=off",
      };
  ...
  private void verifyThatConfigurerSetStatelessSessionCreationPolicy() throws Exception {
    ...
    SessionManagementConfigurer<HttpSecurity> sessionManagementConfigurer =
        mock(SessionManagementConfigurer.class);
  ...
  private void verifyThatConfigurerAddLoginFilter() throws Exception {
    ...
    verify(configuredHttpSecurity, times(2))
        .addFilterAfter(filterCaptor.capture(), filterClassCaptor.capture());
  ...
  void verifyThatConfigurerAddRefreshTokenFilter() {
    ...
    ArgumentCaptor<Class<? extends Filter>> filterClassCaptor =
        ArgumentCaptor.forClass(Class.class);
    verify(configuredHttpSecurity, times(1))
  ...
  void verifyThatConfigurerPermitErrorPath() throws Exception {
    ArgumentCaptor<
        ...
        authorizeHttpRequestsCaptor = ArgumentCaptor.forClass(Customizer.class);
  ...
  void verifyThatConfigurerSetUpLogoutFlow() throws Exception {
    ...
    verify(logoutConfigurer)
        .addLogoutHandler(
            argThat(handler -> handler instanceof RefreshTokenInvalidationLogoutHandler));
