...
class RefreshTokenAuthenticationProviderTest {
  ...
  void setUp() {
    provider =
        RefreshTokenAuthenticationProvider.builder()
  ...
  void authenticateInvalidTokenShouldThrowBadCredentials() {
    doThrow(new BadCredentialsException("Invalid")).when(tokenService).validate(JWT, FINGERPRINT);
  ...
  void authenticateMissingUserShouldThrowInternalError() {
    ...
    assertThrows(
        InternalAuthenticationServiceException.class, () -> provider.authenticate(authentication));
  }
