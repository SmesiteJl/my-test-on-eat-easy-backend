...
class TokenResponseRefreshTokenAuthenticationFilterTest {
  ...
  @Mock private RefreshTokenCookieReader refreshTokenCookieReader;
  @Mock private RequestMapper requestMapper;
  @Mock private AuthenticationSuccessHandler successHandler;
  ...
  void setUp() {
    noCookieFilter =
        TokenResponseRefreshTokenAuthenticationFilter.builder()
  ...
  void attemptAuthenticationWithCookieNoRefreshRequestShouldUseEmptyString() {
    ...
    ArgumentCaptor<RefreshAuthenticationToken> tokenCaptor =
        ArgumentCaptor.forClass(RefreshAuthenticationToken.class);
  ...
  void attemptAuthenticationWithNullCookieShouldUseRefreshRequest() {
    ...
    when(requestMapper.getObjectFromRequest(request, RefreshRequest.class))
  ...
  void attemptAuthenticationWithCookieShouldUseCookieValue() {
    ...
    when(requestMapper.getObjectFromRequest(request, RefreshRequest.class))
        .thenReturn(refreshRequest);
    when(refreshTokenCookieReader.read(request))
        .thenReturn(new Cookie("refresh_token", REFRESH_TOKEN));
