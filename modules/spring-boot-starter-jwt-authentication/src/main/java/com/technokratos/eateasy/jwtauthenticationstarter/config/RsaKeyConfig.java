...
public class RsaKeyConfig {
  ...
  public PublicKey accessPublicKey() throws IOException, GeneralSecurityException {
    return RsaKeysUtils.loadPublicKey(
        jwtProperties.getTokens().getAccess().getKey().getPublicKey());
  }
  ...
  @ConditionalOnProperty(
      prefix = "jwt.tokens",
      name = {"access.key.algorithm", "refresh.key.algorithm"},
      havingValue = "RSA")
  public class ServerOnlyKeyConfig {
    ...
    public PrivateKey accessPrivateKey() throws IOException, GeneralSecurityException {
      return RsaKeysUtils.loadPrivateKey(
          jwtProperties.getTokens().getAccess().getKey().getPrivateKey());
    }
    ...
    public PrivateKey refreshPrivateKey() throws IOException, GeneralSecurityException {
      return RsaKeysUtils.loadPrivateKey(
          jwtProperties.getTokens().getRefresh().getKey().getPrivateKey());
    }
    ...
    public PublicKey refreshPublicKey() throws IOException, GeneralSecurityException {
      return RsaKeysUtils.loadPublicKey(
          jwtProperties.getTokens().getRefresh().getKey().getPublicKey());
    }
  }
}
