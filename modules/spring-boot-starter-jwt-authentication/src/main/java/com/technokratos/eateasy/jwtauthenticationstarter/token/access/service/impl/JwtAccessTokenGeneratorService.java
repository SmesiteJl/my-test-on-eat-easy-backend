 ...
 *
 * <p>Adds standard claims to access tokens:
 *
 * <ul>
 *   <li>User ID (when available via {@link IdentifiableUserDetails})
 *   <li>User authorities
 *   <li>Custom claims from input parameters
 * </ul>
 *
...
public class JwtAccessTokenGeneratorService implements AccessTokenGeneratorService {
   ...
   *
   * <p>Standard claims include:
   *
   * <ul>
   *   <li>userIdClaim - if user implements {@link IdentifiableUserDetails}
   *   <li>authoritiesClaim - normalized list of authorities
   * </ul>
   *
  ...
  private void putUserIdIfPresent(UserDetails userDetails, Map<String, Object> claims) {
    if (!(userDetails instanceof IdentifiableUserDetails<?> identifiable)) {
      log.trace(
          "User details does not implement IdentifiableUserDetails, skipping user id addition");
      return;
    }
