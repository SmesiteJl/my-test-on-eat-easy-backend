 ...
 *
 * <p>Extends {@link AccessTokenUserDetails} with ID capability for audit trails and authorization
 * checks requiring persistent user references.
 *
 * @param <T> type of persistent user identifier (must be {@link Serializable})
 * @see IdentifiableUserDetails
 */
public class AccessTokenIdentifiableUserDetails<T extends Serializable>
    extends AccessTokenUserDetails implements IdentifiableUserDetails<T> {

  private final T userId;

  private AccessTokenIdentifiableUserDetails(
      T userId, String username, Collection<? extends GrantedAuthority> authorities) {
   ...
   */
  public static <T extends Serializable> AccessTokenIdentifiableUserDetails<T> of(
      T userId, String username, Collection<? extends GrantedAuthority> authorities) {
