 ...
 *
 * <p>Produces JSON response with access/refresh tokens and optionally sets refresh token in
 * HTTP-only cookie based on configuration.
 */
@Slf4j
@Builder
...
public class TokenResponseAuthenticationSuccessHandler implements AuthenticationSuccessHandler {
   ...
   *
   * <ol>
   *   <li>Generating tokens
   *   <li>Writing tokens to JSON response
   *   <li>Optionally setting refresh token cookie
   * </ol>
   *
  ...
  public void onAuthenticationSuccess(
    ...
    if (!(principal instanceof UserDetails userDetails)) {
      log.error(
          "Invalid Authentication type. Expected {}, but got {}.",
          UserDetails.class,
          authentication.getClass());
      throw new InternalAuthenticationServiceException(
          "Invalid Authentication type. Expected %s, but got %s."
              .formatted(UserDetails.class, authentication.getClass()));
    }

    TokenResponse tokenResponse = createTokens(userDetails, obtainFingerprint(request));
    log.info(
        "User {} successfully logged in. Generated tokens response: {}",
  ...
  protected String obtainFingerprint(HttpServletRequest request) {
    FingerprintRequest fingerprintRequest =
        requestMapper.getObjectFromRequest(request, FingerprintRequest.class);
  ...
  private void addCookieIfRequired(HttpServletResponse response, TokenResponse tokenResponse)
      throws ServletException, IOException {
    if (!useCookie) {
