 ...
 *
 * <p>Converts Spring Security authentication exceptions to JSON error responses using {@link
 * ErrorResponse} format. Requires concrete implementations to define:
 *
 * <ul>
 *   <li>HTTP status mapping logic
 *   <li>Client-friendly error messages
 * </ul>
 */
@Slf4j
@RequiredArgsConstructor(access = AccessLevel.PROTECTED)
public abstract class AbstractErrorResponseAuthenticationFailureHandler
  ...
  protected ErrorResponse createErrorResponse(String path, HttpStatus status, String error) {
    return ErrorResponse.builder().httpStatus(status).error(error).path(path).build();
  }
}
