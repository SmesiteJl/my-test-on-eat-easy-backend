 ...
 *
 * <p>Contains timestamp, HTTP status code, request path, error description, and optional details
 * payload. Built using the fluent builder pattern.
 */
@Slf4j
@Getter
...
public class ErrorResponse {
   ...
   *
   * <p>Requires {@code httpStatus}, {@code path}, and {@code error} to be set before building. Logs
   * validation failures and throws {@code IllegalStateException} for missing required fields.
   */
  public static class ErrorResponseBuilderImpl
      extends AbstractErrorResponseBuilder<ErrorResponse, ErrorResponseBuilderImpl> {
    ...
    public ErrorResponse build() {
      if (!isHttpStatusSet || error == null || path == null) {
        log.error(
            "Required fields not set: httpStatus={}, error={}, path={}",
            isHttpStatusSet,
            error,
            path);
   ...
   */
  public abstract static class AbstractErrorResponseBuilder<
      E extends ErrorResponse, B extends AbstractErrorResponseBuilder<E, B>> {
