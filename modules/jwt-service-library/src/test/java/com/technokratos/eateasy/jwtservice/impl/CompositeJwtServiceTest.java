...
class CompositeJwtServiceTest {
  ...
  @InjectMocks private CompositeJwtService jwtService;

  private final UserDetails testUser =
      User.builder().username(TEST_USERNAME).password(TEST_PASSWORD).authorities(TEST_ROLE).build();
